...
function _M:run()
	self.player_display = PlayerDisplay.new(0, 200, 200, self.h-200, {0, 0, 0}, "/data/font/VeraMono.ttf", 12)
...
function _M:display(nb_keyframe)
	...
	if self.level and self.level.map and self.level.map.finished then
		...
		self.level.map:minimapDisplay(0, 20, util.bound(self.player.x - 25, 0, self.level.map.w - 50), util.bound(self.player.y - 25, 0, self.level.map.h - 50), 50, 50, 0.6)
	end
	...
	self.key:addBinds
	{
		...
		SHOW_INVENTORY = function()
			...
			d = self.player:showEquipInven("Inventory", nil, "normal", function(o, inven, item, button, event)
				...
				local ud = require("mod.dialogs.UseItem").new(event == "button", self.player, o, item, inven, "normal", function(_, _, _, stop)
					d:generate()
					d:generateList()
